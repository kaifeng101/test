...
const ManagerTablePendingReq = ({ fetchWFHRequests }) => {
  ...
  const handleToggleRow = (request_id) => {
    setOpenRow((prev) => {
      ...
      if (location.pathname.includes("/managerRequestsPage")) {
        if (!isOpen) {
          // Modify the URL by appending the request_id for jackRequestsPage
          navigate(`/managerRequestsPage/${request_id}`, { replace: true });
        } else {
      ...
      else if (location.pathname.includes("/myManagerRequestsPage")) {
        // Toggle the row open/collapse state without changing the URL
        if (!isOpen) {
          // Modify the URL by appending the request_id for jackRequestsPage
          navigate(`/myManagerRequestsPage/${request_id}`, { replace: true });
        } else {
          // Revert to the base URL when collapsing the row.
