...
export default function StaffTablePendingReq({ fetchWFHRequests }) {
  ...
  const handleToggleRow = (request_id) => {
    setOpenRow((prev) => {
      ...
      // If opening the row, append request_id to the URL, else revert the URL.
      if (!isOpen) {
        // Modify the URL by appending the request_id, but stay on the same page.
        navigate(`/staffRequestsPage/${request_id}`, { replace: true });
      } else {
