...
const JackTablePendingReq = ({ fetchWFHRequests }) => {
  // user configuration
  ...
  const handleToggleRow = (request_id) => {
    setOpenRow((prev) => {
      ...
      if (location.pathname.includes("/jackRequestsPage")) {
        if (!isOpen) {
          // Modify the URL by appending the request_id for jackRequestsPage
          navigate(`/jackRequestsPage/${request_id}`, { replace: true });
        } else {
      ...
      else if (location.pathname.includes("/jackPersonalRequestsPage")) {
        // Toggle the row open/collapse state without changing the URL
        if (!isOpen) {
          // Modify the URL by appending the request_id for jackRequestsPage
          navigate(`/jackPersonalRequestsPage/${request_id}`, { replace: true });
        } else {
          // Revert to the base URL when collapsing the row.
